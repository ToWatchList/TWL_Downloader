...
if __name__ == '__main__':
    ...
    try:
        writenfofiles = config.getboolean('twl_downloader_settings', 'writenfofiles')
    except configparser.NoOptionError:
        writenfofiles = False

    try:
        kodihostname = config.get('twl_downloader_settings', 'kodihostname')
    except configparser.NoOptionError:
        kodihostname = None

    try:
        kodiport = config.get('twl_downloader_settings', 'kodiport')
    except configparser.NoOptionError:
        kodiport = None

    try:
        kodiuser = config.get('twl_downloader_settings', 'kodiuser')
    except configparser.NoOptionError:
        kodiuser = None

    try:
        kodipassword = config.get('twl_downloader_settings', 'kodipassword')
    except configparser.NoOptionError:
        kodipassword = None

    try:
        downloadtotmp = config.getboolean('twl_downloader_settings', 'downloadtotmp')
    except configparser.NoOptionError:
        downloadtotmp = True
    ...
    if kodihostname:
        ...
        if not kodiport:
            kodiport = 8080  # Kodi Web default port
        kodiURL = f"http://{kodihostname}:{kodiport}/jsonrpc"
        kodi = Kodi(kodiURL, kodiuser, kodipassword)
        assert kodi.JSONRPC.Ping()['result'] == 'pong',\
            f'\nERROR: bad or response from Kodi @ {f"http://{kodihostname}:{kodiport}/jsonrpc"}'

        if shouldScanKodi or shouldCleanKodi:
            kodi.GUI.ShowNotification({"title": "ToWatchList Downloader",
                                       "message": "New videos downloaded, update Kodi library…"})
        if shouldScanKodi:
            print(f"Scanning Kodi Library @ {kodiURL}")
            kodi.VideoLibrary.Scan()
        if shouldCleanKodi:
            print(f"Cleaning Kodi Library @ {kodiURL}")
            kodi.VideoLibrary.Clean()
        if not shouldCleanKodi and not shouldScanKodi:
            print(f"No Clean/Scan of Kodi Library @ {kodiURL} needed")
